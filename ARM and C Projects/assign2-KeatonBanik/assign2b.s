...
main:
	...
	mov 	x20, 0	//Sets x20 to 1, x20 for loop
	mov	x21, 0		//stores random x21
...
loop:
	...
	add	x20,x20, 1	//increments 1 to x20
	

//Test loops
test:
	cmp 	x20, x19 	// compares current x20 to user input
	b.lt 	loop			// if current x20 is less than user input -> loop, else continue
...
lowFreq:
	...
	cmp	x23, x25 	// compares random number to current highest frequency
	b.gt	highFreq 		// if random number is higher than highest frequency -> x25, else continue
	add 	x26,x26,x23 	// adds random number to sum total
	add	x20,x20, 1	// increments 1 to x20
...
next:
	...
	udiv	x28,x28,x26 		//divides x28 by x26/sum of document, stores back in x28

	ldr	x0, =sizeTxt	//Loads format string for document sum/x26
	mov 	x1, x26	//sets x1 first argument to x26/sum of document
	bl 	printf		//prints format string of document x26/string
